...
<body>
	...
	<main class="relative flex-grow border-b-2"
		...
		<div class="flex justify-center text-jnblack border-b-2 w-full max-w-[1280px] mx-auto mt-15 box-content">
			<div class="fixed w-full bg-white pb-5 pt-2 sm:top-[80px] lg:top-[204px] xl:top-[220px] h-[160px] z-10 mx-auto px-6 md:px-8 2xl:px-16"
			style="top:165px">
				<div
			...
			<div class="pb-10 w-full max-w-[1200px] max-[1200px]:px-6 mx-auto">
				<div class="pt-[160px]">
					...
					<!-- 이벤트 시작 -->
					<ul
						class="md:py-[17px] grid grid-cols-2 gap-5 max-[600px]:block mx-auto md:px-8">
						...
						<c:forEach var="event" items="${ eventList }">
							<li class="w-full aspect-[2.5]">
								<div class="relative h-full">
									...
									<!-- 링크 -->
									<a class="w-full h-full aspect-[2.5] absolute"
										href="event/detail.do?eventcode=${ event.eventcode }"> <!-- 이미지 -->
										<img alt="${ event.title }"
										src="http://localhost/retro_prj/upload/${ event.img2 }"
										decoding="async" data-nimg="fill"
										class="rounded-md overflow-hidden" loading="lazy"
								...
								<div class="flex justify-between w-full px-1 py-3 h-12">
									...
									<!-- 시작/종료 날짜 -->
									<p class="text-[#666666]">${ event.start_date } ~ ${ event.end_date }
									</p>

									<!-- D-DAY -->
									<c:if
										test="${ param.searchType eq 'ING' or param.searchType eq '' or param.searchType eq null }">
										<p class="text-jngreen">D-${ event.dday }</p>
									</c:if>
									<c:if test="${ param.searchType eq 'END' }">
										<p class="text-[#666666]">종료</p>
									</c:if>
								</div>
							</li>
						</c:forEach>
